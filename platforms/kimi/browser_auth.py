...
def detect_auth_surface(*, url: str, text: str) -> AuthSurface:
    ...
    login_markers = (
        "log in to chat with kimi for free", "continue with google",
        "log in with phone number", "verification code", "log in to sync chat history",
    )
...
def register_with_google(
    *, proxy: str | None, email_hint: str, challenge_callback, captcha_solver=None,
    phone_callback=None, chrome_user_data_dir: str = "", chrome_cdp_url: str = "",
    timeout: int = 300, log_fn=print,
) -> dict:
    ...
    with OAuthBrowser(
        ...
        if _is_authenticated(browser):
            ...
            return {"email": email, "cookies": browser.cookie_header(domain_substrings=("kimi.com",)), "storage_state": json.dumps(storage, ensure_ascii=False), "identity_mode": "google"}
        ...
        if not _wait_authenticated(browser, 8, verification=verification):
            ...
            response: ChallengeResponse = challenge_callback(ChallengeRequest(
                kind="oauth_confirmation", message="调试模式：请在可视浏览器中完成 Kimi Google 登录/安全验证",
                url=str(browser.active_page().url or KIMI_URL), metadata={"platform": "kimi", "provider": "google"},
            ))
...
class KimiPhoneRegister:
    ...
    def __init__(self, *, proxy: str | None = None, challenge_callback=None, captcha_solver=None, phone_callback=None, timeout: int = 300, log_fn=print):
        self.proxy = proxy
    ...
    def _debug_or_fail(self, page, message: str) -> None:
        ...
        response: ChallengeResponse = self.challenge_callback(ChallengeRequest(
            kind="security_check", message=f"调试模式：{message}", url=str(page.url or KIMI_URL),
        ...
        with OAuthBrowser(proxy=self.proxy, headless=False, log_fn=self.log) as browser:
            ...
            while time.monotonic() < deadline:
                ...
                if surface is AuthSurface.AUTHENTICATED:
                    if not verification.phone_started or not verification.phone_number:
                        raise RuntimeError("Kimi 手机号注册检测到已有登录会话，拒绝保存为新手机号账号")
                    verification.mark_authenticated()
                    storage = _storage_snapshot(page)
                    phone = verification.phone_number
                    return {"email": phone, "phone_number": phone, "cookies": browser.cookie_header(domain_substrings=("kimi.com",)), "storage_state": json.dumps(storage, ensure_ascii=False), "identity_mode": "phone"}
                if verification.try_handle(page):
                    unknown_cycles = 0
                    time.sleep(0.75)
                    continue
