    ...
    try:
        input_types = tuple(
            page.locator("input").evaluate_all(
                "els => els.map(el => (el.type || el.getAttribute('type') || 'text').toLowerCase())"
            )
        )
    except Exception:
        input_types = ()
...
def _capture_session(browser: OAuthBrowser, *, email_hint: str, password: str = "") -> dict:
    ...
    actual_email = _extract_email(storage)
    email = finalize_oauth_email(actual_email, email_hint, "Z.AI")
    return {
        "email": email,
        "password": password,
        "cookies": browser.cookie_header(domain_substrings=("z.ai",)),
