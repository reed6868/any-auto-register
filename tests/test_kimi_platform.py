...
def test_kimi_is_exposed_with_evidence_backed_registration_capabilities(client):
...
def test_kimi_auth_surface_detection_distinguishes_login_and_authenticated():
    ...
    assert detect_auth_surface(
        ...
        text="Log in to Chat with Kimi for Free Continue with Google Log in with phone number Verification code Send",
...
def test_kimi_plugin_supports_phone_and_artifacts_aware_google_oauth():
    from core.base_platform import RegisterConfig
    from platforms.kimi.plugin import KimiPlatform

    platform = KimiPlatform(config=RegisterConfig(executor_type="headed", extra={"identity_provider": "phone"}))
